...
const password_1 = require("../src/shared/lib/password");
const prisma = new client_1.PrismaClient();
const dayMs = 24 * 60 * 60 * 1000;
const hourMs = 60 * 60 * 1000;
const now = Date.now();
const isDryRun = process.env.SEED_DRY_RUN === '1';
const seedTestPassword = process.env.SEED_TEST_USER_PASSWORD?.trim() || 'change-me-seed-pass-123';
const atOffset = (offsetMs) => new Date(now + offsetMs);
const users = [
    {
        id: '1',
        displayName: 'Admin',
...
const stations = [
    {
        ...
        type: client_1.StationType.QUIZ,
    ...
    },
    {
        ...
        type: client_1.StationType.TIME,
    ...
    },
    {
        ...
        type: client_1.StationType.POINTS,
...
const realizations = [
    {
        ...
        type: client_1.RealizationType.OUTDOOR_GAMES,
        ...
        peopleCount: 18,
        positionsCount: 4,
        status: client_1.RealizationStatus.DONE,
    ...
    },
    {
        ...
        type: client_1.RealizationType.HOTEL_GAMES,
        scenarioId: 's-3',
        teamCount: 6,
        requiredDevicesCount: 8,
        peopleCount: 24,
        positionsCount: 6,
        status: client_1.RealizationStatus.IN_PROGRESS,
    ...
    },
    {
        ...
        type: client_1.RealizationType.WORKSHOPS,
        scenarioId: 's-1',
        teamCount: 3,
        requiredDevicesCount: 5,
        peopleCount: 14,
        positionsCount: 3,
        status: client_1.RealizationStatus.PLANNED,
...
const eventLogs = [
    {
        realizationId: 'r-1',
        actorType: client_1.EventActorType.ADMIN,
...
async function main() {
    if (isDryRun) {
        console.log(`Seed dry run: ${users.length} users, ${stations.length} stations, ${scenarios.length} scenarios, ${realizations.length} realizations.`);
        return;
    }
    ...
    const seededUsers = await Promise.all(users.map(async (user) => ({
        ...
        passwordHash: user.passwordHash
            ? await (0, password_1.hashPassword)(user.passwordHash)
            : user.passwordHash,
    })));
    ...
    console.log(`Seed completed: ${seededUsers.length} users, ${stations.length} stations, ${scenarios.length} scenarios, ${realizations.length} realizations.`);
}
main()
    .catch((error) => {
