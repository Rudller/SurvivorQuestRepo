...
type JoinCodeQrScannerOverlayProps = {
  visible: boolean;
  onClose: () => void;
  onDetected: (rawValue: string) => void;
};

const JOIN_CODE_QR_SCANNER_TEXT: Record<
  UiLanguage,
  {
    title: string;
    subtitle: string;
...
> = {
  polish: {
    ...
    cameraAccessDescription: "Aby zeskanować kod QR realizacji od administratora, włącz dostęp do kamery.",
  ...
  english: {
    ...
    cameraAccessDescription: "Enable camera access to scan the realization QR code from the administrator.",
  ...
  ukrainian: {
    ...
    cameraAccessDescription: "Щоб зіскенувати QR-код реалізації від адміністратора, увімкніть доступ до камери.",
  ...
  russian: {
    ...
    cameraAccessDescription: "Чтобы отсканировать QR-код реализации от администратора, включите доступ к камере.",
    enableCamera: "Включить камеру",
  },
};

export function JoinCodeQrScannerOverlay({ visible, onClose, onDetected }: JoinCodeQrScannerOverlayProps) {
  const adaptiveLayout = useAdaptiveLayout();
  const uiLanguage = useUiLanguage();
  const text = JOIN_CODE_QR_SCANNER_TEXT[uiLanguage];
  ...
  const canScan = useMemo(
    () => visible && permission?.granted && !isScanLocked,
    [isScanLocked, permission?.granted, visible],
  );
